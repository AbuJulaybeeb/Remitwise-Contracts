...
use soroban_sdk::{
    ...
    Address, Env, IntoVal, String, TryFromVal, Symbol, Val, Vec as SorobanVec,
};
...
fn test_goal_created_event_schema() {
    ...
    assert_eq!(remitwise_events.len(), 1, "Should emit exactly one Remitwise GOAL_CREATED event");
    
    let event = remitwise_events.get(0).unwrap();
    
    // Topic Schema: [Remitwise, State, Medium, created]
    assert_eq!(event.1.get(0).unwrap(), symbol_short!("Remitwise").into_val(&env));
    assert_eq!(event.1.get(1).unwrap(), (EventCategory::State as u32).into_val(&env));
    assert_eq!(event.1.get(2).unwrap(), (EventPriority::Medium as u32).into_val(&env));
...
fn test_funds_added_event_schema() {
    ...
    assert_eq!(event.1.get(1).unwrap(), (EventCategory::Transaction as u32).into_val(&env));
    assert_eq!(event.1.get(3).unwrap(), symbol_short!("funds_add").into_val(&env));
...
fn test_funds_withdrawn_event_schema() {
    ...
    let id = client.create_goal(&user, &String::from_str(&env, "Withdraw Test"), &1000, &10000);
    ...
    assert_eq!(event.1.get(3).unwrap(), symbol_short!("funds_rem").into_val(&env));
...
fn test_goal_completed_event_schema() {
    ...
    let completed_event = events.iter().find(|e| {
        e.1.len() == 1 && e.1.get(0).unwrap() == GOAL_COMPLETED.into_val(&env)
    }).expect("GoalCompletedEvent not found");
    
    let data: GoalCompletedEvent = GoalCompletedEvent::try_from_val(&env, &completed_event.2).unwrap();
    assert_eq!(data.goal_id, id);
    assert_eq!(data.owner, user);
    assert_eq!(data.name, name);
...
fn test_batch_add_to_goals_events() {
    ...
    let contributions = SorobanVec::from_array(&env, [
        ContributionItem { goal_id: id1, amount: 200 },
        ContributionItem { goal_id: id2, amount: 300 },
    ]);
    ...
    let event1_data: FundsAddedEvent = FundsAddedEvent::try_from_val(&env, &add_events.get(0).unwrap().2).unwrap();
    ...
    let event2_data: FundsAddedEvent = FundsAddedEvent::try_from_val(&env, &add_events.get(1).unwrap().2).unwrap();
